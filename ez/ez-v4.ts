...
const measurementFile = Deno.args[0];
...
async function mainThread() {
  ...
  const results: any[] = await Promise.all(workers)
...
function runMeasurementWorker(filename: string, name: string, workerData: any) {
...
function buffer2intfloat(buffer: any, length: any) {
  let number = 0;
